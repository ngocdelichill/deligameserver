function Game() {
	...
	this.auction = function() {
		...
		var text = "<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Auction <span id='propertyname'></span></div><div>Highest Bid = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, it is your turn to bid.</div>";
		if(player[currentbidder]._id == _me._id)
			text += "<div><input id='bid' title='Enter an amount to bid on " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Bid' onclick='game.auctionBid();' title='Place your bid.' /><input type='button' value='Pass' title='Skip bidding this time.' onclick='game.auctionPass();' /><input type='button' value='Exit Auction' title='Stop bidding on " + s.name + " altogether.' onclick='if (confirm(\"Are you sure you want to stop bidding on this property altogether?\")) game.auctionExit();' /></div>";
...
function roll(die1 = 0,die2 = 0) { 
	...
	if(p._id == _me._id){
		...
		xhttp.open("POST", `/plays/roll`, true);
		xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhttp.send(`die1=${die1}&die2=${die2}`);
	}
	...
	if (die1 == die2 && !p.jail) {
		...
		console.log("End turn and advance to the next player.");
...
function play() {
	...
	if (game.auction()) {
		...
		xhttp.open("GET", `/plays/auction`, true);
		xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhttp.send();
		return;
	}
	...
	if(p._id == _me._id){
		...
		roll.type = "button";
		roll.id = "nextbutton";
		roll.value = "Roll Dice";
		roll.title = "Roll the dice and move your token accordingly.";
		roll.addEventListener("click",function(){
			game.next();
		});
		document.getElementById("main_control").appendChild(roll);
	...
	if(p._id != _me._id){
		...
		xhttp.open("POST", `/plays/next`, true);
		xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhttp.send(`player=${p._id}`);
	}
	
}

function setup() {
	pcount = parseInt(document.getElementsByClassName("player-input").length);
